...
function MatchCard({
  ...
  return (
    <div className="flex hover:bg-muted/30 transition-colors border-b border-border/20">
      ...
      <div
        className={cn(
          "w-1 flex-shrink-0 group-hover:bg-electric transition-colors",
          isLive && "bg-electric glow-cyan",
          isPendingConfirmation && "bg-amber-500",
          !isLive && !isPendingConfirmation && "bg-transparent group-hover:bg-electric/50"
        )}
      />
      ...
      >
        {/* Time/Status Column */}
        <div className="w-12 flex-shrink-0 text-center flex flex-col items-center justify-center">
          {isLive && (
            <span className="text-[11px] font-bold text-electric glow-text animate-pulse">LIVE</span>
          )}
          {isPendingConfirmation && (
            <span className="text-[11px] font-medium text-amber-500">PEND</span>
          )}
          {isScheduled && (
            <span className="text-[11px] text-muted-foreground font-medium bg-muted/40 px-1.5 py-0.5 rounded">
              {match.scheduled_at ? formatTime(match.scheduled_at) : "-"}
            </span>
          )}
          {isCompleted && (
            <span className="text-[10px] text-muted-foreground font-bold bg-muted/50 px-1.5 py-0.5 rounded">FT</span>
          )}
        </div>
        ...
        <div className="flex-1 min-w-0 text-[13px]">
          ...
          <div className="flex items-center justify-between leading-snug py-0.5">
            ...
            <span
              className={cn(
                "font-bold ml-3 w-6 text-right text-sm",
                isCompleted && match.winner_id === match.player1?.id && "text-electric bg-electric/10 px-1 rounded-sm",
          ...
          <div className="flex items-center justify-between leading-snug py-0.5">
            ...
            <span
              className={cn(
                "font-bold ml-3 w-6 text-right text-sm",
                isCompleted && match.winner_id === match.player2?.id && "text-electric bg-electric/10 px-1 rounded-sm",
                !isCompleted && "text-foreground/80"
              )}
            >
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5">
          <span className="text-xs font-semibold truncate">{tournament.name}</span>
          {isLive && (
            <Circle className="h-2 w-2 fill-electric text-electric flex-shrink-0 animate-pulse glow-cyan" />
          )}
        </div>
        {tournament.venue?.name && (
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        <div className="flex gap-5">
          ...
          <main className="flex-1 min-w-0">
            ...
            <div className="flex items-center justify-between mb-4 card-dark rounded-xl p-2.5 shadow-sm border border-border/20">
              <div className="flex items-center gap-2">
                <button
                  onClick={goToToday}
                  className={cn(
                    ...
                    feedStats?.live
                      ? "bg-electric/20 text-electric border border-electric/30 glow-text"
                ...
                >
                  <span className="flex items-center gap-2">
                    {feedStats?.live ? (
                      <>
                        <Circle className="h-2 w-2 fill-electric text-electric animate-pulse shadow-[0_0_8px_rgba(0,191,191,0.8)] rounded-full" />
                        {feedStats.live} LIVE
                      </>
                    ) : (
                      "Today"
            ...
            {feedStats && feedStats.total_matches > 0 && (
              <div className="flex items-center gap-3 mb-3 text-xs text-muted-foreground">
                ...
                {feedStats.completed > 0 && (
                  <>
                    <span className="text-border">|</span>
                    <span className="text-green-500">{feedStats.completed} completed</span>
                  </>
                )}
              </div>
            )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {filteredTournaments.map((tournament: Tournament) => (
                    <Link
                      key={tournament.id}
                      href={`/tournaments/${tournament.id}`}
                      className="card-dark rounded-2xl p-5 hover:border-electric/30 transition-all card-hover group"
                    >
                      <div className="flex items-start justify-between mb-3">
                        <h3 className="font-bold text-foreground group-hover:text-electric transition-colors">{tournament.name}</h3>
